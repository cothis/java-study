package day016.baseball.main;

import day016.baseball.controller.Controller;
...
public class Main {
	...
	private void menu() {
		int choice;
		while(true) {
			choice = getUserInt("1.선수 등록  2.선수 삭제  3.선수 검색  4.선수 수정  5.선수 모두 출력  6.순위 보기  7.데이터 저장  8.종료", 1, 8);
	...
	private void allPrint() {
		...
		for (Human human : controller.allPrint()) {
			System.out.println(human);
		}
	}

	private void sortList() {
		System.out.println("선수 순위 출력 화면입니다.");
		System.out.println("선수 타입을 입력해주세요.");
		int type = 0;
		while(true) {
			type = getUserInt("1.투수  2.타자", 1, 2);
			if (type == 1) {
				break;
			} else if (type == 2) {
				break;
		...
		List<Human> list = controller.getRating(type);
		for (Human el : list) {
